...
export const viewCart = async (req, res) => {
    try {
        ...
        // Combine product info with quantity
        const products = cart.items.map((item) => ({
            ...item.product,           // spread product details
            quantity: item.quantity,   // add quantity
            totalPrice: item.totalPrice // optionally include total for this item
        }));
...
export const increaseQuant = async (req, res) => {
    try {
        ...
        //increasing quantity
        const increasedItem = await prisma.cartItem.update({
        ...
        const finalAddedItem = await prisma.cartItem.update({
            where: { item_id: currentItem.item_id },
...
export const decreaseQuant = async (req, res) => {
    try {
        ...
        if (quantity.quantity >= 1) {
            const decreasedItem = await prisma.cartItem.update({
                where: { item_id: currentItem.item_id },
                data: {
